...
func (b *BulkOperationPacked16) DecodeInts(blocks, values []int64, iterations int) {
...
func (b *BulkOperationPacked16) DecodeBytes(blocks []byte, values []int64, iterations int) {
	blocksOffset, valuesOffset := 0, 0
	for j := 0; j < iterations; j++ {
		block0 := int64(blocks[blocksOffset]) << 8
		block1 := int64(blocks[blocksOffset+1])
...
func (b *BulkOperationPacked16) DecodeByteToInt(blocks []byte, values []int32, iterations int) {
	...
	for j := 0; j < iterations; j++ {
		...
		block1 := int32(blocks[blocksOffset+1])
		values[valuesOffset] = block0 | block1
		valuesOffset++
		blocksOffset += 2
	}
