...
func (p *Packed16ThreeBlocks) Get(index int) int64 {
	o := index * 3
	return int64(p.blocks[o]&0xFFFF)<<32 |
		int64(p.blocks[o+1]&0xFFFF)<<16 |
		int64(p.blocks[o+2]&0xFFFF)
}

func (p *Packed16ThreeBlocks) GetBulk(index int, arr []int64) int {
	gets := Min(p.valueCount-index, len(arr))
	end := (index + gets) * 3

	off := 0
	for i := index * 3; i < end; i += 3 {
		arr[off] = int64(p.blocks[i]&0xFFFF)<<32 |
			int64(p.blocks[i+1]&0xFFFF)<<16 |
			int64(p.blocks[i+2]&0xFFFF)
		off++
	}
	return gets
}

func (p *Packed16ThreeBlocks) Set(index int, value int64) {
	off := index * 3
...
func (p *Packed16ThreeBlocks) SetBulk(index int, arr []int64) int {
...
func (p *Packed16ThreeBlocks) Fill(fromIndex, toIndex int, value int64) {
