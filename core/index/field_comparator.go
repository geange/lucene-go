...
// setTopValue This method is called by TopFieldCollector to notify the FieldComparator of the top most value, which is used by future calls to LeafFieldComparator.compareTop.
...
type FieldComparator interface {
	...
	// Returns: any N < 0 if slot2's value is sorted after slot1, any N > 0 if the slot2's value is sorted
	// before slot1 and 0 if they are equal
	Compare(slot1, slot2 int) int

	// SetTopValue Record the top value, for future calls to LeafFieldComparator.compareTop. This is only
