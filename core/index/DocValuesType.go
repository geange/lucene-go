...
const (
	...
	// DOC_VALUES_TYPE_SORTED A pre-sorted byte[]. Fields with this type only store distinct byte values
	...
	// DOC_VALUES_TYPE_SORTED_NUMERIC A pre-sorted Number[]. Fields with this type store numeric values in
	// sorted order according to Long.compare(long, long).
	DOC_VALUES_TYPE_SORTED_NUMERIC

	// DOC_VALUES_TYPE_SORTED_SET A pre-sorted Set<byte[]>. Fields with this type only store distinct byte
