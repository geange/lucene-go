...
const (
	...
	FIELDS_VERSION_START         = 1
	FIELDS_VERSION_OFFHEAP_INDEX = 2
	FIELDS_VERSION_META          = 3 // Version where all metadata were moved to the meta file.
	FIELDS_VERSION_NUM_CHUNKS    = 4 // Version where numChunks is explicitly recorded in meta file and a dirty chunk bit is recorded in each chunk
	FIELDS_VERSION_CURRENT       = FIELDS_VERSION_NUM_CHUNKS
	FIELDS_META_VERSION_START    = 0
...
func NewStoredFieldsWriter(ctx context.Context, directory store.Directory, si index.SegmentInfo, segmentSuffix string,
	...
	err = codecs.WriteIndexHeader(ctx, metaStream, INDEX_CODEC_NAME+"Meta", FIELDS_VERSION_CURRENT, si.GetID(), segmentSuffix)
	if err != nil {
		return nil, err
	}
	...
	err = codecs.WriteIndexHeader(ctx, fieldsStream, formatName, FIELDS_VERSION_CURRENT, si.GetID(), segmentSuffix)
	if err != nil {
		return nil, err
	}
