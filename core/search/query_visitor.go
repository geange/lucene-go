...
type QueryVisitor interface {
	...
	// ConsumeTerms Called by leaf queries that match on specific terms
	//Params:
	...
	// AcceptField Whether or not terms from this field are of interest to the visitor Implement this to
	// avoid collecting terms from heavy queries such as TermInSetQuery that are not running on fields of interest
	AcceptField(field string) bool

	// Pulls a visitor instance for visiting child clauses of a query The default implementation returns this, unless occur is equal to BooleanClause.Occur.MUST_NOT in which case it returns EMPTY_VISITOR
	//Params:
