...
func MergeTopDocs(ctx context.Context, start, topN int, shardHits []index.TopDocs, setShardIndex bool) (index.TopDocs, error) {
	return mergeAux(ctx, nil, start, topN, shardHits, setShardIndex)
}

// Auxiliary method used by the merge impls.
// A sort value of null is used to indicate that docs should be sorted by score.
func mergeAuxTopDocs(ctx context.Context, sort index.Sort, start, size int, shardHits []index.TopDocs, setShardIndex bool) (index.TopDocs, error) {
	...
	if sort == nil {
		...
		for shardIDX := 0; shardIDX < len(shardHits); shardIDX++ {
			...
			if shard.GetTotalHits().GetRelation() == index.GREATER_THAN_OR_EQUAL_TO {
...
func merge(ctx context.Context, sort index.Sort, start, topN int,
	shardHits []index.TopDocs, setShardIndex bool) (*TopFieldDocs, error) {

	if sort == nil {
		return nil, errors.New("sort must be non-null when merging field-docs")
	}

	docs, err := mergeAux(ctx, sort, start, topN, shardHits, setShardIndex)
	if err != nil {
		return nil, err
	}
	return docs.(*TopFieldDocs), nil
}

// Auxiliary method used by the merge impls. A sort value of null is used to indicate that docs should be sorted by score.
func mergeAux(ctx context.Context, sort index.Sort, start, size int,
	shardHits []index.TopDocs, setShardIndex bool) (index.TopDocs, error) {

	var queue structure.PriorityQueueInterface[*ShardRef]
	if sort == nil {
		queue = NewScoreMergeSortQueue(shardHits)
	} else {
		queue = NewMergeSortQueue(sort, shardHits)
	}
...
func NewMergeSortQueue(sort index.Sort, shardHits []index.TopDocs) *MergeSortQueue {
	panic("")
}
